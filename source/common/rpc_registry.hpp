...
namespace TrRpc
{
    namespace server
    {
        ...
        public:
            ...
            {
                INF_LOG("PDManager::onServiceRequest invoked, rid=%s, method=%s, optype=%d", svr_req->rid().c_str(), svr_req->method().c_str(), (int)svr_req->optype());
                if (!svr_req) { ERR_LOG("ServiceRequest 为空，动态转型失败"); return; }
                // 服务操作请求：服务注册/服务发现/
